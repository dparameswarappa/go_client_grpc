...
func Display(ctx context.Context, query Query, cfg *Config) error {
	...
	for _, n := range resp.GetNotification() {
	    prefix := ""
		for _, k := range n.Prefix.GetElement() {
			prefix += "/" + k
		}
		cfg.Display([]byte(strconv.Itoa(count)))
	    cfg.Display([]byte("timestamp:" + strconv.FormatInt(n.Timestamp, 10)))
		cfg.Display([]byte("prefix:" + prefix)) 
	    for _, m := range n.GetUpdate() {
		     path := ""
		    for _, p := range m.Path.GetElement() {
				path += p
			}
			cfg.Display([]byte(path + ":" + string(m.Value.Value)))
		}
		count += 1
		cfg.Display([]byte("------------------------------------------------------------"))
	}
...
func DisplayStream(ctx context.Context, query Query, cfg *Config) error {
	...
	for {
		...
		case *ocpb.SubscribeResponse_Update:
			fmt.Print("---------------\n")
			//for _, n := range v.Update() {
	    		prefix := ""
				for _, k := range v.Update.Prefix.GetElement() {
					prefix += "/" + k
				}
				cfg.Display([]byte("Timestamp:" + strconv.FormatInt(v.Update.Timestamp, 10)))
				cfg.Display([]byte("Prefix:" + prefix))
				for _, m := range v.Update.GetUpdate() {
					path := ""
					for _, p := range m.Path.GetElement() {
						path += p
					}	
